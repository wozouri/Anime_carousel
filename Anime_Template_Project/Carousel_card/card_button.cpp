...
void Card_button::paintEvent(QPaintEvent* event)
{
    ...
    painter.drawRoundedRect(0, -3, 122, 32, 8, 8);
     ...
     painter.setPen(pen);
    QPoint points[] = { QPoint( 28, 10), QPoint( 33, 15), QPoint( 28, 20), };
...
{
    if (m_button_color == newButton_color) return;
    m_button_color = newButton_color;
    update(); emit button_colorChanged();
}
...
{
    if (m_color_opacity == newColor_opacity) return;
    m_color_opacity = newColor_opacity;
    update(); emit color_opacityChanged();
}
