...
{
    if (this->isClicked == true) mousePos = event->pos();
}
...
void Carrier_card::circle_reset()
{
    TimerTimeout++;
    update();
    if (TimerTimeout == 15) display_circle = true;
    if (TimerTimeout < 87 && display_circle == true) m_circle_degrees -= 5;
    if (TimerTimeout == 87)
    {
        restore_disc();
...
void Carrier_card::set_gradient_color()
{
    ...
    clr.setAlpha(100); m_color1 = clr;
    clr.setAlpha(68); m_color2 = clr;
    clr.setAlpha(255); m_color3 = clr;
}
QColor Carrier_card::getMostFrequentColor(const QPixmap& pixmap)
{
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = 0; x < width; ++x) {
            ...
            float h, s, l;
            color.getHslF(&h, &s, &l);
            if (s < minSaturation && l > maxLightnessForWhite) continue;
            colorCounts[rgb]++;
        }
    }
    if (colorCounts.isEmpty()) return QColor();
...
{
    if (m_carrier_card_id == newCarrier_card_id) return;
...
{
    if (m_circle_degrees == newCircle_degrees) return;
